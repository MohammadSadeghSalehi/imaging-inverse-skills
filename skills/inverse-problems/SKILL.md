...
| The object is a few point sources (fluorophores, stars, spikes) | Off-the-grid sparse recovery (BLASSO) solved by sliding Frank–Wolfe, in `references/communities.md`. A pixel-grid `L1Prior` biases positions and splits sources. |
| The forward operator is known only up to a bound, or only through input-output pairs | The imperfect-operator methods of Korolev and coauthors, or the approximation-error model, in `references/communities.md`. Do not absorb operator error into a larger noise level. |
...
- For several parameters at once, descend on SURE with its gradient with respect to the parameters (SUGAR), in the bilevel table of the imaging-optimisation reference. For sparse analysis and TV regularisers, the degrees of freedom SURE needs are available in closed form (Vaiter, Deledalle, Peyré, Dossal, and Fadili, Applied and Computational Harmonic Analysis 35(3), 2013, doi:10.1016/j.acha.2012.11.006).
...
If the question is whether one structure in the reconstruction is real, a hypothesis test by convex optimisation (BUQO, in `references/communities.md`) answers it without sampling. For posterior expectations under an expensive PDE forward model, multilevel Monte Carlo cuts the sampling cost; it is in the same reference.

...
description: >
  ...
  Chambolle. Implementations are DeepInverse, ASTRA, CIL, and LION.
  Use when the user
  ...
  tomography, statistical inversion, limited-angle tomography, shearlets,
  D-bar, learned primal-dual, unrolled, waveform inversion,
  Noise2Inverse, or runs /inverse-problems.
---
...
| A small paired set should set `λ`, the filters, or the potentials of a variational model | Bilevel learning. If the inner problem is nonsmooth, differentiate a fixed number of primal-dual iterations (Ochs, Ranftl, Brox, Pock); the technique is in the imaging-optimisation algorithms reference. A variational network is the MRI form of the same unroll. |

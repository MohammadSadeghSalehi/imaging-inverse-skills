...
- Smooth data term, a nonsmooth term on `Kx`, and a constraint: Condat–Vũ, which uses the gradient of the data term and needs no prox of it. Step condition `1/τ − σ‖K‖² > L/2`. There is no class in 0.4.2; the example in the reference is a few lines.
- Curvature varying strongly across the image, or a nonconvex edge-preserving penalty without a closed-form prox: variable-metric forward-backward and majorise–minimise (Chouzenoux, Pesquet, Repetti). Very many unknowns: random block-coordinate updates (Combettes and Pesquet).
- A large image with a natural coarse version: multilevel forward-backward or IML FISTA, reporting the fine-level objective.
- Convergence is slow and then suddenly fast on a sparse, low-rank, or TV problem: that is finite identification of the structure followed by local linear convergence (Liang, Fadili, Peyré). Do not stop at the slow phase.
...
- The data term is a long sum and a denoiser replaces the prox: one subset of measurements per data step (online plug-and-play). The convergence claim is still one of the hypotheses in `../inverse-problems/references/communities.md`.
...
`λ`, a Fields-of-Experts filter bank, or a sampling mask is an outer variable. The inner problem stays the variational reconstruction. If that inner problem is nonsmooth, differentiate a fixed run of primal-dual iterations (Ochs, Ranftl, Brox, Pock); the procedure is in `references/algorithms.md`. If it is smooth, unroll it (`unfold=True`, `trainable_params`) or use the implicit-function route (`DEQConfig` on `GD`, `PGD`, or `HQS`). Which hypergradient to compute, and what each supports, is the decision table in the bilevel section of the reference: unrolling for a nonsmooth or fixed-depth inner problem, implicit differentiation for a smooth strongly convex one, MAID when the inner solve dominates the cost, inexact stochastic gradients when there are many training pairs, and piggyback primal-dual differentiation for a nonsmooth convex inner problem. A fixed small number of inner iterations gives a biased hypergradient; say so if that is what you ran.
